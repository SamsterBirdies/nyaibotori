...
module.exports = {
	...
	async execute(interaction) {
		...
		const replies_avatar =
		[
			` Also, what even is your pfp? Is that your mom?`,
			` Ugh... and your avatar makes me wanna vomit.`,
			` Also, what kind of stupid pfp is that?`,
			` Why would you even identify yourself as this.. uh.. whatever this image is???`,
			` Also, this pic aint hot btw.`,
			` Your picture also looks like my left hand drew it.`,
			`... cringe pfp too.`,
			` I also request that you change your profile picture... for your own good.`,
			` Also, BRYUH YOUR PFP! KILL IT! KILL IT WITH FIRE!!!`,
		];
		//text builder
		var hide = false;
		...
		if (user.id == nyaibotori_id) {
			...
			if (Math.random() < 1/2) {
				final_text += replies_avatar[Math.floor(Math.random() * replies_avatar.length)];
			}
		}
		//send
		if (show_avatar) {
			await interaction.reply({ content: final_text, ephemeral: hide, files: [avatar + ".png"]});
		} else {
